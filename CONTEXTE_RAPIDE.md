...
- **Stratégie d'hébergement à coût minimal (2026-06-22, révisée)** : PowerSync **managé** (~250-300 $/mois) identifié comme la charge la plus lourde, disproportionnée pour le pilote en Afrique. Révision après recherche : PowerSync a une édition **« Open Edition » self-hébergeable gratuite** (Docker `journeyapps/powersync-service`, licence FSL→Apache 2.0 déjà effective) — **pas besoin de coder une synchro maison**, `app/lib/core/sync/` ne change pas, seul l'hébergement du service change. Décision : **pilote** = self-hébergement Postgres+Auth + service PowerSync Open Edition sur VPS mutualisé (~10-25 $/mois) ; **avant la mise en prod réelle** = bascule possible vers Supabase Cloud et/ou PowerSync Cloud managés (migration facile/réversible, juste changement d'URL de connexion) — ou rester self-hébergé si l'économie reste prioritaire. Piste boîtier local par pharmacie (Raspberry Pi, résilience totale hors-ligne) envisagée en V2/V3 si le volume justifie l'investissement matériel, non retenue pour le pilote initial.
