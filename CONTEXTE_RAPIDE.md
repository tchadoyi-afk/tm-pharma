...
- **Stratégie d'hébergement à coût minimal (2026-06-22)** : PowerSync (~250-300 $/mois) identifié comme la charge la plus lourde, disproportionnée pour le pilote en Afrique (marché très sensible au coût récurrent en $). Décision : **pilote** = self-hébergement Postgres+Auth sur VPS mutualisé (~10-20 $/mois) + **PowerSync remplacé par une synchro maison** (à écrire dans `app/lib/core/sync/`) ; **avant la mise en prod réelle** = bascule vers Supabase Cloud managé (Pro, ~25 $/mois), migration facile/réversible (même schéma, juste changement d'URL de connexion). Piste boîtier local par pharmacie (Raspberry Pi, résilience totale hors-ligne) envisagée en V2/V3 si le volume justifie l'investissement matériel, non retenue pour le pilote initial.
