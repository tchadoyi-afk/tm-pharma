...
### Réapprovisionnement (MVP = assisté ; V2 = brouillon auto-déclenché — validation manuelle toujours requise)
...
- **V2** : génération **automatique** des brouillons de commande (déclenchée par seuil, sans intervention manuelle pour la créer) + envoi direct intégré (EDI/API fournisseur) — **la validation manuelle avant envoi au fournisseur reste obligatoire à tous les paliers (MVP, V2, V3)**, jamais d'envoi automatique sans confirmation humaine.
...
> **Gestion des fournisseurs = MVP, et complète (décision 22/06/2026, étendue le 23/06/2026)**, pas V2 — c'est la base de l'approvisionnement (réception de stock, bons de commande), donc une dépendance directe du tronc commun. Carnet d'adresses fournisseurs (nom/téléphone/email) géré dans l'écran `Fournisseurs` sous permission `supplier.manage` (lecture sous `stock.view`). **Extension 23/06/2026** : le **portail fournisseurs** (échange structuré de bons de commande/réception, suivi de statut côté fournisseur via l'abstraction `SupplierConnector`) est lui aussi avancé au MVP, pour offrir une gestion fournisseurs complète dès le pilote — seuls la **création automatique des brouillons de commande** (déclenchement par seuil, sans saisie manuelle du brouillon) et l'**EDI/API direct** avec les systèmes fournisseurs restent en V2. **Règle invariante (décision 23/06/2026) : la validation manuelle d'une commande avant envoi au fournisseur est obligatoire à tous les paliers (MVP/V2/V3) — aucune automatisation ne doit permettre l'envoi d'une commande sans confirmation humaine explicite.**
